...
# ADR-018: Authoring method — caller-agnostic skill injected into a lean executor; eval-first disciplines, scoped
...
1. **One-way authoring dependency: caller-agnostic skill -> lean executor.** Knowledge
   ...
   ([ADR-017](ADR-017-agent-design-contract.md)), the main session injects the relevant skill
