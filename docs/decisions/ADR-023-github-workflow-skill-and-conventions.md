...
> **Superseded by [ADR-024](ADR-024-git-workflows-domain-command-agent-skill.md).** The *conventions*
> below (Conventional Commits, no attribution, full-loop scope, compose `nxtlvl:review`) carry forward
> unchanged; what ADR-024 revisits is the *shape* — from a single in-context skill to a three-layer
> `command → agent → skill` domain with an isolated executor. (Originally drafted as ADR-012, then
> ADR-015; renumbered to **ADR-023** on merge because each prior number was already taken by an
> unrelated decision accepted on `main` first. The decision content is unchanged.)

# ADR-023: Standardize the GitHub workflow as a vendored skill that composes the review agent — Conventional Commits, no attribution
