...
# ADR-014: Evaluation — agent-self-evaluation as an advisory per-task done-condition check; formal eval suites stay reactive; gates unchanged

## Context
Phase 3 (Evaluate) of the harness-review build method.
[ADR-013](ADR-013-agent-authoring-method.md) deferred one question: how the "done-condition
...
4. **Formal eval suites stay reactive.** The eval-first principle is already adopted
   ([ADR-013](ADR-013-agent-authoring-method.md)). Standing pass@k / regression suites
...
- Rejected: adopt the principle ([ADR-013](ADR-013-agent-authoring-method.md)), defer the
...
- The self-eval discipline is itself authored as a caller-agnostic skill injected into the
  executor ([ADR-013](ADR-013-agent-authoring-method.md)), not baked into each agent.
