...
# ADR-016: Orchestration — adopt the gated delegate-don't-inline pipeline; own the composition, never the dispatch runtime; scoped roster

## Context
Phase 5 (Orchestrate) of the harness-review build method. This is where the operating model
fixed in [ADR-012](ADR-012-agent-design-contract.md) — a lean orchestrator delegating to
...
and dispatch runtime (the chain-runner, the router table, the control pane). ADR-012 makes the
...
1. **Adopt the gated, size-classified, delegate-don't-inline pipeline as nxtlvl's orchestration
   ...
   ([ADR-013](ADR-013-agent-authoring-method.md)).
2. **Own the composition; never build the dispatch runtime
   ...
   roster ([ADR-012](ADR-012-agent-design-contract.md)) — not a tag->chain lookup table, a
...
3. **Adapt the composition-side patterns.** Self-contained delegation briefs — acceptance
   ...
   Phase 3), which is skill injection ([ADR-013](ADR-013-agent-authoring-method.md)) plus a clear
   ...
   [ADR-014](ADR-014-agent-evaluation-model.md) self-evaluation and
   [ADR-015](ADR-015-agent-debugging-model.md) self-debug. Delegation also isolates context
...
- Cons: a persistent multi-agent team runtime — reconstructed orchestration, and aimed at a squad
  ...
  ([ADR-012](ADR-012-agent-design-contract.md) scope).
...
- nxtlvl has one operating shape: a gated, size-classified pipeline whose every phase is a
  delegation to a scoped specialist, with human gates after Plan and before Commit — the ADR-012
...
- Delegation briefs, parallel-fan-out synthesis, and dynamic discovery become standing
  ...
  [ADR-014](ADR-014-agent-evaluation-model.md) / [ADR-015](ADR-015-agent-debugging-model.md).
