...
| [015](ADR-015-github-workflow-skill-and-conventions.md) | GitHub workflow standardized — a vendored skill composing `nxtlvl:review`; Conventional Commits, no attribution | Superseded by ADR-016 |
| [016](ADR-016-git-workflows-domain-command-agent-skill.md) | git-workflows shipped as a three-layer `command → agent → skill` domain with an isolated, edit-less executor (`git-workflow-runner`) | Accepted |

> **Numbering note:** this branch jumps 012 → 015 because ADR-012 *(agents execute, skills hold
> knowledge)*, ADR-013 *(confident-core capability domains)*, and ADR-014 *(quality-first over leanness)*
> were accepted on `main` after this branch was cut; they merge in alongside 015–016. The original
> ADR-012 on this branch was renumbered to **ADR-015** to avoid colliding with `main`'s ADR-012.
