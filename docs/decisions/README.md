...
| [031](ADR-031-labs-in-sandbox-topology.md) | `agents-lab` and `evals-lab` live as tracked subdirs under `Developer/sandbox/nxtlvl-labs/` (not separate repos, not gitignored scratch); graduation is an in-repo `git mv`; records the 2026-06-22 relocation away from the original separate-`~/agent-lab`-repo design | Accepted (build in-progress) |
