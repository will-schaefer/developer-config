...
# ADR-017: Operation — nxtlvl runs as gated, interactive, single-operator sessions; reject autonomous/continuous/enterprise runtimes; reuse the existing metric, gate, and kill switches
...
([ADR-016](ADR-016-agent-orchestration-model.md)). This decides nxtlvl's operating mode and what,
...
2. **Reject autonomous, continuous-loop, and enterprise-fleet operation as the operating mode.**
   ...
   ([ADR-016](ADR-016-agent-orchestration-model.md)) and the invoked-not-continuous gate
...
3. **Adapt the transferable disciplines into decisions already made.** Loop failure modes (churn
   ...
   ([ADR-015](ADR-015-agent-debugging-model.md)) plus stop-and-ask and model-escalation discipline
   ([ADR-013](ADR-013-agent-authoring-method.md)) at operate time. The failure-class distribution
   ...
   the review/security gate ([ADR-016](ADR-016-agent-orchestration-model.md)).
...
- Cons: reconstructs a runtime and a memory knowledge-graph
  ...
  ([ADR-016](ADR-016-agent-orchestration-model.md),
...
- Cons: an unattended loop contradicts the gated, interactive mode; its real value (failure modes,
  recovery) is already covered by [ADR-015](ADR-015-agent-debugging-model.md) /
  [ADR-013](ADR-013-agent-authoring-method.md) / [ADR-016](ADR-016-agent-orchestration-model.md).
