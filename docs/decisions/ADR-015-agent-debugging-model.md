...
# ADR-015: Debugging — adopt the introspection self-debug loop; scope the architecture audit to owned layers; diagnostic, never a gate
...
1. **Adopt `agent-introspection-debugging` as the in-session self-debug loop (adapted).** A
   caller-agnostic skill ([ADR-013](ADR-013-agent-authoring-method.md)) the orchestrator or a
   ...
   ([ADR-006](ADR-006-hook-fail-open-gated-blocking.md)). It pairs with ADR-013 stop-and-ask
   (when to halt) and [ADR-014](ADR-014-agent-evaluation-model.md) self-evaluation (quality
   reflection).
