...
# ADR-024: Ship the GitHub workflow as a three-layer git-workflows domain — command → agent → skill — with an isolated, edit-less executor

## Context

[ADR-023](ADR-023-github-workflow-skill-and-conventions.md) standardized the GitHub loop as a single
...
2. **nxtlvl now has agents.** `doc-keeper` landed on `main` as nxtlvl's first agent; ADR-023's premise
...
That reopens the agent-vs-skill question ADR-023 closed — and ADR-023's "no sandbox to express" claim
...
- **Skill `github-workflow`** (refined in place, not replaced) — the caller-agnostic knowledge:
  ...
  merge` loop, language-plural. It names no caller; the conventions from ADR-023 carry forward unchanged.
...
**Why an agent is now justified** — refuting ADR-023's decisive point. The agent-vs-skill axis
([scoping doc](../reference/ecc-agent-vs-skill-scoping.md) §5) says the tell is *isolation / a
restricted tool sandbox / autonomy / a model tier*. ADR-023 conflated "writes via `git`" with "needs
...
### Keep it a single in-context skill (ADR-023's shape)
...
- Supersedes [ADR-023](ADR-023-github-workflow-skill-and-conventions.md) (conventions unchanged; only the
